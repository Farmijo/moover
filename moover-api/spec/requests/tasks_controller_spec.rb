...
RSpec.describe Api::TasksController, type: :request do
  ...
  describe 'GET /api/tasks' do
    context 'with valid authentication' do
      it 'returns all tasks' do
        # Create tasks directly in the test instead of using let!
        create(:task, :pack)
        create(:task, :clean)
        create(:task, category: 'admin', is_room_specific: true)
        create(:task, category: 'logistics', is_room_specific: false)

        get '/api/tasks', headers: headers, as: :json

        expect_json_response(200)
        expect(json_response).to be_an(Array)
        expect(json_response.size).to be >= 4  # Al menos las tareas que creamos + las de seeds
      end
      ...
      context 'with category filter' do
        it 'filters tasks by category' do
          get '/api/tasks?category=pack', headers: headers, as: :json
          
          expect_json_response(200)
          expect(json_response.size).to be >= 1  # Al menos una tarea de la categoría pack
          expect(json_response.all? { |t| t['category'] == 'pack' }).to be true
        end

        it 'returns empty array for non-existent category' do
          get '/api/tasks?category=nonexistent', headers: headers, as: :json
      ...
      context 'with room_specific filter' do
        it 'filters room-specific tasks' do
          get '/api/tasks?room_specific=true', headers: headers, as: :json
        ...
        it 'filters general tasks' do
          get '/api/tasks?room_specific=false', headers: headers, as: :json
      ...
      context 'with timing filters' do
        ...
        it 'filters tasks by timing_before' do
          get '/api/tasks?timing_before=0', headers: headers, as: :json
        ...
        it 'filters tasks by timing_after' do
          get '/api/tasks?timing_after=0', headers: headers, as: :json
  ...
  describe 'filtering combinations' do
    ...
    it 'combines category and room_specific filters' do
      get '/api/tasks?category=pack&room_specific=true', headers: headers, as: :json
    ...
    it 'returns empty result for incompatible filters' do
      get '/api/tasks?category=nonexistent&room_specific=true', headers: headers, as: :json
